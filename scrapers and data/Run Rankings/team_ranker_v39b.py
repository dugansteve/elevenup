...
"""
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
SOCCER TEAM RANKING SYSTEM - V39 (BOYS + PLAYERS + STATES)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

V39 CHANGES FROM V38:
  ✅ BOYS SUPPORT - Now ranks Boys age groups (B08-B13) in addition to Girls
...
USAGE:
  python team_ranker_v39.py                        # Interactive mode
  python team_ranker_v39.py --cleanup              # Force cleanup first  
  python team_ranker_v39.py --no-cleanup           # Skip cleanup
  python team_ranker_v39.py --dry-run              # Preview cleanup only
  python team_ranker_v39.py --verbose              # Extra diagnostics
...
class TeamRankerV30:
    ...
    def __init__(self, db_path='../seedlinedata.db', verbose=False):
        ...
        self.all_age_groups = [
            'G13', 'G12', 'G11', 'G10', 'G09', 'G08/07',  # Girls
            'B13', 'B12', 'B11', 'B10', 'B09', 'B08'      # Boys
        ]
        self.cleanup_stats = {
            'bad_teams_removed': 0, 
            'names_cleaned': 0,
    ...
    def calculate_rankings(self, games_df):
        ...
        for team, stats in team_stats.items():
            ...
            league = stats['league']
            league_factor = self.LEAGUE_FACTORS.get(league, 0.90)
    ...
    def run_all_age_groups(self, do_cleanup=None, dry_run=False):
        ...
        print(f"\n{'='*80}")
        print("🏆 SOCCER TEAM RANKING SYSTEM V31")
        print(f"{'='*80}")
        print(f"Database: {self.db_path}")
    ...
    def save_outputs(self, all_rankings):
        ...
        # ═══════════════════════════════════════════════════════════════════
        # SAVE EXCEL - V39: With improved state lookup
        # ═══════════════════════════════════════════════════════════════════
            ...
            for age_group, rankings in all_rankings.items():
                for rank, (team, stats) in enumerate(rankings, 1):
                    all_rows.append({
                        ...
                        'Team': team,
                        'Age Group': age_group,
                        'Gender': 'Boys' if age_group.startswith('B') else 'Girls',  # V39
                        'League': stats['league'],
                        ...
                        'State': self.get_team_state(team, stats.get('conference', '')),  # V39: Use get_team_state
                        ...
                        'Avg GD': f"+{stats['goal_diff']/stats['games_played']:.2f}" if stats['goal_diff'] > 0 else f"{stats['goal_diff']/stats['games_played']:.2f}",
                        'Games': stats['games_played'],
                    })
            
            df = pd.DataFrame(all_rows)
            excel_path = Path(f'All_Age_Groups_Rankings_V39_{timestamp}.xlsx')
...
def main():
    parser = argparse.ArgumentParser(
        description='Soccer Team Ranking System V31 - Comprehensive Duplicate Fix',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python team_ranker_v31.py                        # Interactive
  python team_ranker_v31.py --cleanup              # Force cleanup first  
  python team_ranker_v31.py --no-cleanup           # Skip cleanup
  python team_ranker_v31.py --dry-run              # Preview cleanup only
  python team_ranker_v31.py --verbose              # Show detailed diagnostics
  python team_ranker_v31.py path/to/data.db        # Specify database

V31 Changes:
  - Preserves database capitalization (MVLA, HTX, VDA)
  - Comprehensive team aliases (50+ patterns)
  - Fixes duplicate game counting (Solar SC G09: 65 -> ~30 games)
        """
    )
