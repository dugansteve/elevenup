...
USAGE:
  python cleanup_database_v3.py                    # Interactive mode
  python cleanup_database_v3.py --dry-run          # Preview changes only
  python cleanup_database_v3.py --auto             # Auto-confirm all changes
  python cleanup_database_v3.py path/to/db.db      # Specify database path
...
def cleanup_database(db_path, dry_run=False, auto_confirm=False, verbose=True):
    ...
    Example:
        from cleanup_database_v3 import cleanup_database
        stats = cleanup_database('seedlinedata.db', auto_confirm=True)
...
Examples:
  ...
  python cleanup_database_v3.py --auto             # Auto-confirm all changes
  python cleanup_database_v3.py path/to/db.db      # Specify database path
  python cleanup_database_v3.py --dry-run --auto   # Preview with full analysis
        """
    )
    ...
    if args.db_path:
        ...
        if not db_path:
            ...
            print("   Example: python cleanup_database_v3.py /path/to/seedlinedata.db")
            sys.exit(1)
