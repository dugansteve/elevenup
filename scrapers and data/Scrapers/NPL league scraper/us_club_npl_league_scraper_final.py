#!/usr/bin/env python3
"""
US Club Soccer NPL League Scraper v21.0
=======================================
...
- NEW: disambiguate_team_name() adds state identifiers based on region/league context
- FIX: Teams like "Beach FC" now get "(CA)" or "(VA)" suffix based on region
- WHY: Prevents duplicate entries for teams with same name in different states

Changes in v21.0:
- FIXED: Age group conversion now applies to CSV output (was only in database save)
  CSV now shows G12, B13, etc. instead of birth years like 2013, 2012
- NEW: Smart deduplication with fuzzy team name matching
- NEW: normalize_team_for_id() for consistent team matching across sources
- NEW: WAL mode and busy_timeout for better database concurrency
- All v20 features preserved (resume capability, status tracking)
...
═══════════════════════════════════════════════════════════════════════════════
  ...
  1. SMART DEDUPLICATION (save_npl_game_to_db function):
     - Uses 3-step matching: exact game_id → fuzzy match → insert/update/skip
     - Fuzzy matching uses league LIKE '%NPL%' to match all NPL regional leagues
  ...
  2. TEAM NAME NORMALIZATION (normalize_team_for_id function):
     - Removes: FC, SC, Academy, Club, United, Soccer, NPL, Premier, Elite
     - Removes birth year patterns: 2013G, 2014B
     - Removes all non-alphanumeric characters, truncates to 30 chars
     - PRESERVE: All remove_patterns and remove_words lists
     - WHY: NPL team names often include league and year suffixes

  3. AGE GROUP CONVERSION (v19/v21 critical fix):
     - Converts 4-digit birth year to G##/B## format
     - "2014" → "G14" or "B14" based on gender
     - PRESERVE: The birth year conversion in save_npl_game_to_db()
     - WHY: GotSport returns birth years; database needs standard age format

  4. RESUME CAPABILITY (v20 feature):
     - npl_scrape_status table tracks which leagues have been scraped
     - --scrape flag processes pending leagues only
     - --scrape-all re-scrapes everything
     - PRESERVE: Status tracking functions and table schema
  ...
  5. DATABASE RESILIENCE:
     ...
     - PRESERVE: PRAGMA statements in save_npl_game_to_db()
     - WHY: Multiple scrapers may access the database simultaneously

  6. LOCATION SCRAPING (v19 feature):
     - Extracts city, state, zip from team profile pages
     - PRESERVE: Location extraction logic in scrape_team_profile()
     - WHY: Provides geographic data for ranking and display
"""
...
# =============================================================================
# AMBIGUOUS TEAM NAME DISAMBIGUATION (v21b)
# =============================================================================
AMBIGUOUS_TEAMS = {
    'beach fc': {
        'states': {
            'CA': ['Southwest', 'SoCal', 'Southern California', 'West'],
    ...
    'sporting': {
        'states': {
            'CA': ['Southwest', 'SoCal', 'NorCal', 'West'],
...
def disambiguate_team_name(team_name: str, context_region: str = None,
                           context_league: str = None) -> str:
    """
    Add state identifier to ambiguous team names based on region/league context.
    V21b: Prevents duplicate team entries by consistently adding state identifiers.
    """
    if not team_name:
        return team_name
    ...
    # Try to infer from region or league
    context_lower = ''
    if context_region:
        context_lower = context_region.lower()
    if context_league:
        context_lower += ' ' + context_league.lower()

    for state_code, regions in states_info.items():
        for region in regions:
            if region.lower() in context_lower or context_lower in region.lower():
...
def save_npl_team_to_db(db_path: str, team: Dict) -> bool:
    ...
    # Infer state from team name/region/league (do this first for both insert and update)
    inferred_state = infer_state_from_npl_team(team.get('team_name'), team.get('region'), team.get('league'))
    ...
    if existing:
        ...
        cur.execute("""UPDATE teams SET
                      ...
                      WHERE team_url = ?""",
                   (team.get('team_name'), team.get('club_name'), final_state, team.get('region'), team_url))
        conn.commit()
        conn.close()
        return False
    ...
    # v21 FIX: Convert 4-digit birth year to Gxx/Bxx format
    ...
    if age_group and age_group.isdigit() and len(age_group) == 4:
        birth_year_suffix = age_group[2:]  # "2014" → "14"
        gender_prefix = 'B' if gender.lower().startswith('b') else 'G'
        age_group = f"{gender_prefix}{birth_year_suffix}"  # "G14" or "B14"
...
def normalize_team_for_id(team: str) -> str:
    ...
    remove_words = ['soccer', 'futbol', 'fc', 'sc', 'academy', 'club', 'united', 'npl']
    ...
    normalized = re.sub(r'\s*\d{4}[GB]\s*', '', normalized)  # 2013G, 2014B
...
def save_npl_game_to_db(db_path: str, game: Dict) -> bool:
    """Save NPL game to games table with smart deduplication.
    ...
    home_team = game.get('home_team') or game.get('home_team_normalized', '')
    away_team = game.get('away_team') or game.get('away_team_normalized', '')
    ...
    if not existing:
        cur.execute("""
            ...
            WHERE game_date_iso = ? AND age_group = ? AND league LIKE '%NPL%'
        """, (game_date_iso, age_group))
    ...
    if existing:
        existing_has_scores = existing[1] is not None and existing[2] is not None
        ...
        if new_has_scores and not existing_has_scores:
            ...
            cur.execute("""UPDATE games SET
                ...
                WHERE rowid = ?""",
                (game.get('home_score'), game.get('away_score'), existing[0]))
            conn.commit()
            conn.close()
            return True  # Updated
        else:
...
class CSVExporter:
    ...
    def write_game(self, game: Dict) -> bool:
        ...
        gender = game.get('gender', 'Girls')
        if age_group and str(age_group).isdigit() and len(str(age_group)) == 4:
            birth_year_suffix = str(age_group)[2:]  # "2014" → "14"
            gender_prefix = 'B' if str(gender).lower().startswith('b') else 'G'
            row['age_group'] = f"{gender_prefix}{birth_year_suffix}"  # "G14" or "B14"
...
class GotSportScraper:
    ...
    def _parse_game_row(self, row, league: LeagueConfig, age_group: str,
        ...
        # V21b: Disambiguate team names that exist in multiple states
        home_team = disambiguate_team_name(home_team, context_region=league.region, context_league=league.name)
        away_team = disambiguate_team_name(away_team, context_region=league.region, context_league=league.name)
