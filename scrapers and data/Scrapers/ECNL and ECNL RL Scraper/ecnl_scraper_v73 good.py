...
"""
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
ECNL COMPREHENSIVE SCRAPER v73 - ADMIN UI COMPATIBLE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

V73 CHANGES:
  ✅ FIX: Player count now shows when --players flag is used (not just debug)
  ✅ NEW: CSV export for TEAMS (ECNL_teams_*.csv)
  ✅ NEW: CSV export for PLAYERS (ECNL_players_*.csv)
  ✅ Tracks all teams and players scraped for CSV export
  ✅ Shows player stats in final summary
...
USAGE:
  # Scrape with players and CSV exports:
  python ecnl_scraper_v73.py --scrape --players --limit 10
  
  # Full scrape with visible browser:
  python ecnl_scraper_v73.py --reset-status --scrape-all --players
...
def export_teams_csv(teams: List[Dict], output_dir: Path, prefix: str) -> str:
    """V73: Export teams to CSV file"""
    if not teams:
        return ""
    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    fp = output_dir / f"{prefix}_teams_{ts}.csv"
    fields = ['team_url', 'team_name', 'club_name', 'age_group', 'gender', 'league', 'conference',
...
def export_players_csv(players: List[Dict], output_dir: Path, prefix: str) -> str:
    """V73: Export players to CSV file"""
    if not players:
        return ""
    ts = datetime.now().strftime('%Y%m%d_%H%M%S')
    fp = output_dir / f"{prefix}_players_{ts}.csv"
    fields = ['player_name', 'first_name', 'last_name', 'team_name', 'team_url', 
...
class ECNLScraper:
    def __init__(self, db_path: str, headless: bool = False, debug: bool = False, 
        ...
        self.all_games = []
        self.all_teams = []      # V73: Track teams for CSV export
        self.all_players = []    # V73: Track players for CSV export
        self.errors = 0
            ...
            for i, team in enumerate(teams, 1):
                ...
                try:
                    ...
                    if games:
                        ...
                        self.all_teams.append(team_data)  # V73: Track for CSV export
                        ...
                        if self.include_players:
                            ...
                            if players:
                                ...
                                # V73: Track players with team info for CSV export
                                for p in players:
...
async def main_async():
    ...
    if args.scrape or args.scrape_all or args.reset_status:
        ...
        if teams:
            ...
            # V73: Export all CSV files
            if scraper.all_games:
                csv_file = export_csv(scraper.all_games, OUTPUT_DIR, 'ECNL')
                if csv_file:
                    print(f"📄 Games CSV: {csv_file}")
            
            if scraper.all_teams:
                teams_csv = export_teams_csv(scraper.all_teams, OUTPUT_DIR, 'ECNL')
                if teams_csv:
                    print(f"📄 Teams CSV: {teams_csv}")
            
            if scraper.all_players:
                players_csv = export_players_csv(scraper.all_players, OUTPUT_DIR, 'ECNL')
                if players_csv:
