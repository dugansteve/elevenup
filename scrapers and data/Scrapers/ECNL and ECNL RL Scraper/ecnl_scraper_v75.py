...
"""
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
ECNL COMPREHENSIVE SCRAPER v75 - IMPROVED ADDRESS EXTRACTION
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

V75 CHANGES:
  ✅ FIXED: Address/City/State/ZIP extraction completely rewritten
  ✅ Now finds text directly below club name (heading element)
  ✅ Finds 5-digit ZIP first (most reliable marker)
  ✅ Handles full state names: Georgia -> GA, Colorado -> CO
  ✅ Handles lowercase state codes: Co -> CO
  ✅ Strips newlines from all extracted values
  ✅ Handles website formatting errors (duplicate state names)

V74 CHANGES:
  ✅ CSV files created at START of scraping (not end)
  ✅ Data appended INCREMENTALLY as each team is scraped
  ✅ All 3 CSV files: games, teams, players

V72 CHANGES:
  ✅ CRITICAL FIX: Fixed JavaScript regex escaping
...
OUTPUT FILES (created at start, updated incrementally):
...
TEAMS_CSV_FIELDS = ['team_url', 'team_name', 'club_name', 'age_group', 'gender', 'league', 'conference',
                    'state', 'city', 'street_address', 'zip_code', 'official_website', 'event_id']
PLAYERS_CSV_FIELDS = ['player_name', 'first_name', 'last_name', 'team_name', 'team_url', 
                      'jersey_number', 'position', 'graduation_year', 'age_group', 'league']
...
def append_to_csv(filepath: Path, data: List[Dict], fields: List[str]) -> int:
    """V74: Append data rows to existing CSV file"""
...
class ECNLScraper:
    ...
    async def scrape_teams(self, teams: List[Dict]) -> Tuple[int, int]:
        """Scrape games for a list of teams with incremental CSV writes"""
            ...
            for i, team in enumerate(teams, 1):
                ...
                try:
                    ...
                    if games:
                        ...
                        # V74: Write games to CSV incrementally
                        if self.games_csv_path:
                            append_to_csv(self.games_csv_path, games, GAMES_CSV_FIELDS)
                        ...
                        # V74: Write team to CSV incrementally
                        if self.teams_csv_path:
                            append_to_csv(self.teams_csv_path, [team_data], TEAMS_CSV_FIELDS)
                        ...
                        if self.include_players:
                            ...
                            if players:
                                ...
                                # V74: Write players to CSV incrementally
                                if self.players_csv_path and player_data_for_csv:
                                    append_to_csv(self.players_csv_path, player_data_for_csv, PLAYERS_CSV_FIELDS)
