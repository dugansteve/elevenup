...
"""
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
ECNL COMPREHENSIVE SCRAPER v71 - ADMIN UI COMPATIBLE
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

V71 CHANGES:
  ✅ CRITICAL FIX: Replaced broken HTML table parsing with text-based extraction
  ✅ TGS website doesn't use HTML table rows - must parse raw page text
  ✅ Restored working extraction logic from v58/v63 scraper
  ✅ Uses page.evaluate() with JavaScript to parse document.body.innerText
  ✅ Properly extracts dates, opponents, scores, home/away indicators
...
V68 CHANGES:
  ...
  ✅ Improved league normalization (accepts ecnl, ecnl-rl, both lowercase)
...
ADMIN UI ARGUMENTS:
  ...
  --scrape-all                Scrape all teams (ignore pending status)
...
USAGE:
  ...
  python ecnl_scraper_v71.py --gender both --ages 13,12,11 --no-confirm
  
  # Reset and re-scrape all ECNL teams:
  python ecnl_scraper_v71.py --reset-status --league ECNL --no-confirm
  
  # Interactive mode:
  python ecnl_scraper_v71.py
  
  # Original commands still work:
  python ecnl_scraper_v71.py --scrape --limit 10
  python ecnl_scraper_v71.py --stats
...
def find_database_path():
    search_paths = [
        # v68: Added parent.parent for actual folder structure:
...
def get_teams_from_db(db_path: str, league: str = None, age: str = None, 
    ...
    # Normalize league value (v68)
    if league:
        league_upper = league.upper().replace('-', ' ')
        if league_upper == 'ECNL RL':
            league_normalized = 'ECNL RL'
        elif league_upper == 'ECNL':
            league_normalized = 'ECNL'
        else:
            league_normalized = None  # 'both' or invalid = no filter
    else:
        league_normalized = None
    
    if league_normalized:
...
def reset_scrape_status(db_path: str, league: str = None, gender: str = None, age_list: List[str] = None) -> int:
    """Reset scrape status to 'pending' for teams matching filters. Returns count of teams reset."""
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
    
    # Build query based on filters
    q = "UPDATE discovered_urls SET scrape_status = 'pending', last_scraped = NULL WHERE league IN ('ECNL', 'ECNL RL')"
    params = []
    ...
    if league:
        league_upper = league.upper().replace('-', ' ')
        if league_upper == 'ECNL RL':
            q = "UPDATE discovered_urls SET scrape_status = 'pending', last_scraped = NULL WHERE league = ?"
            params = ['ECNL RL']
        elif league_upper == 'ECNL':
            q = "UPDATE discovered_urls SET scrape_status = 'pending', last_scraped = NULL WHERE league = ?"
            params = ['ECNL']
        # else: 'both' or invalid = reset all ECNL leagues
    
    # Gender filter
    if gender and gender.lower() != 'both':
        gender_value = 'Girls' if gender.lower() == 'girls' else 'Boys'
        q += " AND gender = ?"
        params.append(gender_value)
...
class ECNLScraper:
    ...
    async def scrape_team_details(self, page, team: Dict) -> Dict:
        ...
        try:
            ...
            if location_elem:
                ...
                if location_text:
                    # Parse city, state from location text (uses global re import)
    ...
    async def scrape_team_schedule(self, page, team: Dict) -> List[Dict]:
        """
        Scrape games from a team's schedule page using text extraction.
        V71 FIX: TGS website doesn't use HTML table rows - must parse raw page text.
        This approach is from the working v63 scraper.
        """
        games = []
        url = team.get('url')
        if not url:
        ...
        try:
            ...
            # V71: Use JavaScript to parse raw page text (TGS doesn't use HTML tables)
            games_data = await page.evaluate("""
                () => {
                    ...
                    // Method: Look for date patterns and extract surrounding context
                    ...
                    dateMatches.forEach((match, idx) => {
                        ...
                        // Find end point - next date match or 300 chars
                        let endIdx;
                        ...
                        // V71/v60 FIXED OPPONENT EXTRACTION
                        // TGS format: "Team Name ECNL RL G08/07" or "Team Name ECNL G09"
                        let opponent = null;
                        
                        // Pattern 1: "Team Name ECNL RL G08/07" or "Team Name ECNL RL G08"
                        let opponentMatch = afterContext.match(/([A-Za-z][A-Za-z0-9\s()\-'&.]+?)\s+ECNL\s+RL\s+[GB]\d{2,4}(?:\/\d{2,4})?/i);
                        if (opponentMatch) {
                            opponent = opponentMatch[1].trim();
                        }
                        
                        // Pattern 2: "Team Name ECNL G08/07" or "Team Name ECNL G08" (no RL)
                        if (!opponent) {
                        ...
                        // Pattern 3: Look for team name after venue/location line
                        if (!opponent) {
                        ...
                        // Pattern 4: Fallback - any text followed by ECNL
                        if (!opponent) {
                        ...
                        // Look for score pattern - TGS shows scores in RESULTS column
            ...
            # Convert extracted games to the format expected by save_games
            ...
            for g in games_data:
                ...
                # Determine home/away teams based on the team we're scraping
                if home_away == 'Home':
                    home_team = team_name
                    ...
                    # Unknown - default to scraped team as home
                    home_team = team_name
                    away_team = opponent if opponent else 'Unknown'
            ...
            if self.debug and games:
                games_with_opponent = sum(1 for g in games if g.get('away_team') != 'Unknown' and g.get('home_team') != 'Unknown')
                games_with_score = sum(1 for g in games if g.get('home_score') is not None)
                print(f"\n    📅 Found {len(games)} games: {games_with_opponent} with opponent, {games_with_score} with scores")
...
async def main_async():
    ...
    parser.add_argument('--league', choices=['ECNL', 'ECNL RL', 'ecnl', 'ecnl-rl', 'both'], 
                       help='Specific league (ECNL, ECNL RL, or both)')
    ...
    parser.add_argument('--reset-status', action='store_true', 
                       help='Reset scrape status to pending before scraping (use with --scrape-all)')
    
    args = parser.parse_args()
    
    db = args.db or DATABASE_PATH
    print("\n" + "="*60)
    print("🏃 ECNL COMPREHENSIVE SCRAPER v68")
    print("="*60)
    print(f"📂 Database: {db}")
    ...
    if args.league:
        ...
        print(f"🏆 League: ECNL + ECNL RL (both)")
    if args.gender:
    ...
    if args.scrape_all:
        print(f"🔄 Mode: Re-scrape all teams")
    elif args.scrape:
        print(f"🔄 Mode: Pending teams only")
    ...
    if should_scrape:
        ...
        # Reset status if requested (v68)
        if args.reset_status:
            reset_count = reset_scrape_status(db, args.league, args.gender, age_list)
            print(f"🔄 Reset {reset_count} teams to pending status")
        
        teams = get_teams_from_db(db, args.league, args.age, 
                                  pending_only=not args.scrape_all and not args.reset_status,
                                  limit=args.limit,
                                  gender=args.gender,
                                  age_list=age_list)
